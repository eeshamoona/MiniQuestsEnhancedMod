...
import com.example.miniquestsenhancedmod.item.ModItems;
...
@Mod(MiniQuestsEnhancedMod.MODID)
public class MiniQuestsEnhancedMod {
    public static final String MODID = "miniquestsenhancedmod";
    public static final Logger LOGGER = LogUtils.getLogger();

    public MiniQuestsEnhancedMod(IEventBus modEventBus, ModContainer modContainer) {
    ...
    private void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
        if (!event.getEntity().level().isClientSide) {
            ...
            LOGGER.info("[MiniQuestsEnhanced] Gave {} the testing tokens on login.", event.getEntity().getName().getString());
        }
    }
